...
async def apply_to_job_generic(job_url: str, info: dict, resume_path: str) -> str:
    """
    Generic job application function that works with any job application URL.

    Args:
        job_url: URL of the job application page
        info: Dict with user info:
            - first_name, last_name, email, phone
            - postal_code, country, city, address, state
            - US_citizen (bool), sponsorship_needed (bool)
            - Optional: age, gender, race, Veteran_status, disability_status
        resume_path: Path to resume PDF file

    Returns:
        Summary of application result
    """
    llm = ChatOpenAI(model='gpt-5.1')  # Use stable model
    ...
    # Load base prompt and build task with dynamic info
    base_prompt = load_prompt("web_agent.txt")
...
- Postal Code: {info.get('postal_code', '')}
- Country: {info.get('country', 'USA')}
- US Citizen: {info.get('US_citizen', True)}
- Sponsorship Needed: {info.get('sponsorship_needed', False)}
- Age: {info.get('age', '')}
- Gender: {info.get('gender', 'Prefer not to say')}
- Race: {info.get('race', 'Prefer not to say')}
- Veteran Status: {info.get('Veteran_status', 'Prefer not to say')}
- Disability Status: {info.get('disability_status', 'Prefer not to say')}
...
async def apply_to_rochester_regional_health(info: dict, resume_path: str):
	...
	llm = ChatOpenAI(model='gpt-4.1')  # Use stable model
    ...
    - Navigate to https://bdo.wd3.myworkdayjobs.com/en-US/BDO/job/Toronto---Bay-St/AI-Engineer---Generative---Agentic-AI_JR5256?source=LinkedIn
...
if __name__ == '__main__':
	parser = argparse.ArgumentParser(description='Apply to AI Engineer job')
